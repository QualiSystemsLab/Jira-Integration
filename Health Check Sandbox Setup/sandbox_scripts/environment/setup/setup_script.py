...
class EnvironmentSetup(object):
    ...
    def _run_health_checks(self, api):
        api.WriteMessageToReservationOutput(self.reservation_id, 'Running health checks...')
        rd = api.GetReservationDetails(self.reservation_id).ReservationDescription
        errors = []
        for r in rd.Resources:
            try:
                self.logger.info("Executing health check command on resource {0}".format(r.Name))
                o = api.ExecuteCommand(self.reservation_id, r.Name, 'Resource', 'health_check', []).Output
                if 'fail' in o.lower():
                    raise CloudShellAPIError(999, o, o)
                api.WriteMessageToReservationOutput(self.reservation_id, o)
            except CloudShellAPIError as exc:
                if exc.code not in (EnvironmentSetup.NO_DRIVER_ERR,
                                    EnvironmentSetup.DRIVER_FUNCTION_ERROR,
                                    MISSING_COMMAND_ERROR):
                    self.logger.error("Error executing health check command on resource {0}. Error: {1}".format(r.Name, exc.rawxml))
